...
export default function SalaryReportPage() {
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        ...
        if (userIdFromQuery && yearFromQuery && monthFromQuery) {
    ...
    // Effect to auto-download PDF if requested from another page
    useEffect(() => {
        ...
        const performDownload = async () => {
            ...
            try {
                ...
                pdf.save(`payslip_${report.user.username}_${selectedYear}-${selectedMonth}.pdf`);
                ...
                // Clean up the URL to remove download param, preventing re-download on refresh.
                const newUrl = `/dashboard/admin/salary-report?userId=${report.user.id}&year=${selectedYear}&month=${selectedMonth}`;
                router.replace(newUrl, { shallow: true });

            } catch (error) {
                console.error("Error auto-generating PDF:", error);
                toast({ title: 'PDF Generation Failed', description: 'Could not auto-generate the payslip PDF.', variant: 'destructive' });
            } finally {
                hideLoader('pdf-generation');
            }
        };

        if (shouldDownload && report && !isGeneratingPdf) {
            performDownload();
        }
    }, [searchParams, report, isGeneratingPdf, router, toast, showLoader, hideLoader, selectedYear, selectedMonth]);
